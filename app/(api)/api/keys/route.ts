...
export async function GET(request: NextRequest) {
	...
	const keys =
		(await kv.hgetall<Record<string, KeyData>>(`user:${user.id}:keys`)) ?? {};
...
export async function POST(request: NextRequest) {
	...
	await kv
		.multi()
		.hset(`user:${user.id}:keys`, { [id]: keyData })
		.set(`key:${hash}`, user.id)
		.exec();
...
export async function DELETE(request: NextRequest) {
	...
	const keyData = await kv.hget<KeyData>(`user:${user.id}:keys`, id);

	if (!keyData) {
		return NextResponse.json({ error: "Key not found" }, { status: 404 });
	}

	await kv.hset(`user:${user.id}:keys`, {
